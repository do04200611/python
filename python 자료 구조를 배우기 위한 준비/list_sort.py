class Node:
   def __init__(self, name, left=None, right=None): # 섬 생성자
      ...
      self.left = left  # 섬의 왼쪽으로 진행 시키기 위해서 선언
      self.right = right # 섬의 오른쪽으로 진행 시키기 위해서 선언
...
def A_course(a):  # A-코스
   if a != None:  # n이 None이 아니면
      print(a.name, '->', end='')  # 섬 n 방문
      A_course(a.left)  # n의 왼쪽 진행
      A_course(a.right) # n의 오른쪽 진행
